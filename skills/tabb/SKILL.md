...
1. 建 `.tabb/locks/`、board 表头、journal；把 `.tabb/` 加入 `.gitignore` 的工作派给 worker。
2. 拆并行任务写入 board；明确依赖，blocked 任务不得提前 spawn。
3. 为每个 worker 嵌入下节完整协议，补齐仓库/分支/口径/测试/边界。
4. 轮询 locks、board、journal 和在途 agent 里程碑：
   - `[DECISION]`：立即带推荐方案上抛用户；裁决写回 journal。
   - `[HANDOFF]`：放行对应 blocked 任务。派单只叫下游自行读该条，不复述正文。
   - `[DONE]+[SEAL]` / `[HANDOFF]`：更新完成状态；锁已释放即可推进。
5. worker 全部完成后关闭它们，释放并发槽；创建唯一 git-ops，先跑全量验证，PASS 后才 seal。
6. 从真实回执汇报结果；不凭记忆宣称完成、提交、推送或生效。
...
手头任务完成后可去 board 认领 open 任务：写入后回读；认领者不是自己则让位。
...
持续回报用 `ScheduleWakeup` 约每 270 秒自续排；每轮读 locks/board/journal，再从在途 agent 的 `TaskOutput` 取最新里程碑。仍有人工作则续排；全员结束且无 open 任务时停止。用户问进度则立即检查。状态表固定为：`任务 / agent / 进度条 / 当前阶段`。进度只能按最新里程碑分段显示，如 `■■■□ 3/4`；无信号写 `？/？ 无里程碑信号` 并补发里程碑要求，禁止编平滑百分比。
