...
3. **默认继承主 agent。** 所有 worker 使用当前主 agent 模型；非只读任务同时继承当前 effort，只读任务固定覆盖为 `low`。tabb 不跨模型路由，也不使用 OpenCode。

### 模型与 effort

| 类别 | 判定 | 运行档 |
|---|---|---|
| `readonly` | 零业务写入，只查代码、日志、资料、调用链 | 当前主模型 / `low` |
| `work` | 实现、测试、修复、git 收口等其他任务 | 当前主模型 / 当前主 effort |

任务规模和推理难度只用于决定拆单与跳数，不再决定模型或 effort。
...
3. 读取当前主 agent 的实际 model/effort，运行路由脚本；输出为 `pool<TAB>model<TAB>effort`，必须原样用于登记：

   ```bash
   python3 <skill-dir>/scripts/route-task.py --class <readonly|work> \
...
- `work`：spawn 省略 `model` 和 `reasoning_effort`，直接继承当前主 agent 的模型与 effort。
- `readonly`：spawn 省略 `model`，传 `reasoning_effort="low"`。Codex 中覆盖 effort 时用 `fork_turns="none"` 或正整数，不用 `all`。

禁止为 tabb worker 显式改传其他模型。assignments 登记路由脚本输出的当前主模型与目标 effort；给用户的派单回执注明 `inherit-current`。
...
调度：class=<readonly|work>；model=<脚本 model>；effort=<脚本 effort>；role=<readonly|implementation|git-ops>。
...
- 每个任务是否先跑路由脚本，非只读继承当前 model/effort、只读继承当前 model 并使用 low？
