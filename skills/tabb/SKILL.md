...
已知同系列强弱：`Sol > Luna > Terra`，`Opus > Sonnet > Haiku`；effort 为 `low < medium < high < xhigh < max < ultra`。未知型号、跨系列或跨代际无法可靠比较时继承主模型。相同档必须写 `inherit-main`，显式值只表示严格降档。
...
3. 按模型路由并在 spawn 前登记。**继承某一维必须写 `inherit-main`，且 spawn 省略该维 override**：同模型不传 `model`，同 effort 不传 `reasoning_effort`/`thinking`。显式值只准严格降档；主档来自用户当前回合明示或可信 runtime metadata，禁止从旧登记、默认值或自身猜测填写：
...
4. 为每个 worker 嵌入下节完整协议。spawn 参数必须逐维匹配登记：`inherit-main` 就省略 override；显式降档才传 override。登记失败不得 spawn，也不得手写 assignments 绕过脚本。
...
调度：model=<inherit-main|严格更弱模型>；effort=<inherit-main|严格更低 effort>；role=<readonly|simple|implementation|git-ops>。
...
- `inherit-main` 维度是否省略了 spawn override，显式维度是否确为严格降档？
