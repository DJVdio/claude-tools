...
> **`[[ta]]` 是真引用，不是修辞。** 跑 `/tabb` 时只有本文被加载进你的上下文，ta 的正文**不会**自动出现。所以凡本文写"沿用 / 同 / 复用 [[ta]]"而没就地展开的地方，你要**自己去读 `~/.claude/skills/ta/SKILL.md`** 把细节捞出来，别因为"上下文里没有"就当它不存在、自己现编一套。
...
复用 [[ta]] 的 git-ops 常驻收口专员，唯一有权 commit/push 的 agent。`.tabb/` 目录不入库。
...
bash ~/.claude/skills/ta/seal.sh --dry-run /tmp/seal.txt
bash ~/.claude/skills/ta/seal.sh /tmp/seal.txt
```
...
| git-ops 逐条发 git 命令做收口 | 固定流水线被拆成 20-30 次秒级 LLM 往返，一次收口拖几分钟；多端串行十几分钟 | 从 journal 的 `[DONE]` 行誊出白名单 → 写清单 → 跑 `~/.claude/skills/ta/seal.sh`，一条 Bash 跑完（含三步核验），多仓并发 |
