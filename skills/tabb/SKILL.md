...
每个 agent 只 append 自己的行，天然不争用；主 agent 和下游 agent 按需读。四类条目，用 tag 前缀区分：
...
6. **驱动收口**：`[SEAL]` 累积到批次阈值或用户说"收口"时，派 git-ops 跑 `seal-from-journal.sh`——它从 journal 的 `[SEAL]` 行自动生成白名单清单并并发收口，**不需要任何人去读 `[DONE]` 长文**。
...
回报格式（完工必做——写 journal 不算回报）：干完这单，**必须以一条 return 消息回报给主 agent**——根因 file:line、改动清单、测试证据(红→绿)、遗留点、里程碑全过 N/N。**你往 journal 写的 [DONE]/[SEAL]/[HANDOFF] 只是给黑板留痕，不等于回报；绝对别写完 journal 就 idle 停住不 return——主 agent 等的就是你这次 return，你一 idle 它只能来催，白费一轮。** 没有下一个 open 任务可拉、也没在等锁，就带着回报 return 结束。
```
...
| 干活 agent 只写 `[DONE]`、不写 `[SEAL]` | git-ops 无机器可读白名单，被迫退回阅读理解 | 派单协议块钉死"完工两行都写"；`[DONE]` 长篇随便写（那些分析有价值），但 `[SEAL]` 一行不能少 |
