...
5. 为 OpenCode 写完整 prompt 文件，然后调用：
   ...
   连续启动所有独立任务，每次必须检查 launch 命令退出码；首个环境级失败就停止继续启动。脚本通过 macOS `launchctl submit` 交给 launchd 托管，worker 不属于一次性 Codex 命令的进程组；状态写入 `.taboc/opencode/`。
6. 高性能 worker 使用 harness 的 spawn 工具并嵌入同一黑板协议。主 agent 不重复其工作。
