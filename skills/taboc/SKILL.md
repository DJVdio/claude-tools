...
用户说“看看任务面板”“任务面板”或“谁在做什么”时，必须运行并原样展示；每项都要显示精确值或可信的 `inherit-main` 继承关系：
...
收尾只核对四件事：OpenCode 是否承接所有非高风险任务；effort 是否按 `medium/high/max` 分级；premium 的 `inherit-main` 是否省略对应 override、显式值是否严格降档；终态、锁、全量 PASS 和收口白名单是否齐全。
