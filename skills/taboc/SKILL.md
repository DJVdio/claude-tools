...
4. 为 OpenCode 写完整 prompt 文件，然后调用：
   ...
   连续启动所有独立任务，不等前一个结束。`launch-opencode.sh` 立即返回，后台 worker 状态写入 `.taboc/opencode/`。
5. 高性能 worker 使用 harness 的 spawn 工具并嵌入同一黑板协议。主 agent 不重复其工作。
6. 轮询用一条命令聚合，避免逐个读大日志：
...
7. `[HANDOFF]` 放行 blocked 下游；`[DECISION]` 立即上抛用户；符合强制升级条件才派高性能接替者。
8. 全部实现有 `[DONE]+[SEAL]`、只读有 `[HANDOFF]`、锁全释放后，创建唯一高性能 git-ops 跑全量验证与收口。
...
只有额度、429/402、容量、模型不可用、网络中断、空响应或 CLI 非零退出才自动换模型。业务测试失败、`[DECISION]`、正常完成不触发模型轮换。失败重试前脚本只清理由同一 worker id 持有的锁，保留已有改动供下一模型检查并续做；因此 prompt 必须要求幂等地检查现状。

`TABOC_MODELS` 是逗号分隔候选列表；`TABOC_MAX_ATTEMPTS` 可限制尝试数。免费优先是硬默认：除非用户显式把付费模型写入 `TABOC_MODELS`，脚本不自行花钱。
...
| 所有模型失败 | 状态为 `exhausted`；按风险升级高性能 worker 或如实报告 |
