...
5. 为 OpenCode 写完整 prompt 文件，然后调用：
   ...
   连续启动所有独立任务，每次必须检查 launch 命令退出码；首个环境级失败就停止继续启动。脚本使用 `launchctl bootstrap` 加载 `KeepAlive=false` 的一次性 LaunchAgent，worker 不属于 Codex 命令的进程组，但退出后不会被自动重启。启动时自动登记 `auto-free` 和请求档位，状态文件随后会覆盖为真实模型/档位。免费 OpenCode 不受主 agent 模型/effort 上限约束。多个 worker 仍并行执行；脚本只短暂串行化模型目录查询和 CLI 冷启动，以避开 OpenCode 共享 SQLite 的 `database is locked`，不会把任务执行改成串行。
...
只有 OpenCode JSONL 中顶层 `type=error` 的结构化错误事件（额度、429/402、容量、模型不可用、网络中断、单次超时等）或 CLI 非零退出才自动换模型。每个模型默认最多运行 300 秒，`TABOC_ATTEMPT_TIMEOUT` 可调整；超时会终止整个进程组并尝试下一个候选，避免长 `retry-after` 卡死执行池。禁止对整段日志搜索 `quota/capacity/402`：任务正文、源码和协议自身可能包含这些词。当次运行新写入的 `[HANDOFF]`/`[DONE]`/`[DECISION]` 是完成权威，即使 CLI 退出码异常也不得重做。若模型只在最终 JSON 文本中输出一条与本 worker id 严格匹配的终态记录，脚本会安全补写 journal；不匹配的自由文本不会被采纳。正常退出却没有终态记录时标记 `incomplete`，只续跑一次，不自动换模型。重启已有终态记录的同 id 会被拒绝；确需人工重试时显式传 `--retry`。

失败重试前脚本只清理由同一 worker id 持有的锁，保留已有改动供下一模型检查并续做；因此 prompt 必须要求幂等地检查现状。

`TABOC_MODELS` 是逗号分隔候选列表；`TABOC_MAX_ATTEMPTS` 可限制尝试数；`TABOC_ATTEMPT_TIMEOUT` 控制单模型秒数；`TABOC_STARTUP_HOLD` 控制 CLI 冷启动错峰秒数。这些变量会由 launcher 透传到一次性 LaunchAgent。实时模型目录暂时失败或为空时，脚本仍尝试内置的已知免费候选，不会产生 `tried=0` 的假耗尽。免费优先是硬默认：除非用户显式把付费模型写入 `TABOC_MODELS`，脚本不自行花钱。免费候选可以强于主 agent；这一例外不得扩展到消耗 Codex/Claude 额度的子 agent。
...
| 模型长时间重试或卡死 | 单次超时后杀掉进程组，自动尝试下一免费模型 |
| 并发冷启动出现 SQLite 锁冲突 | 模型查询与短暂冷启动由原子锁错峰，业务执行继续并行 |
| 实时模型目录为空/失败 | 使用内置已知免费候选继续真实调用，不报 `tried=0` |
| 所有免费模型真实调用失败 | 状态为 `exhausted`；逐任务按风险决定接替，禁止整批升级 |
| `opencode`/launchctl/launchd 环境故障 | 状态为 `blocked`；暂停便宜池、修环境或请用户处理，禁止升级这批任务 |
| OpenCode 进程失联且持锁 | 面板以 run lock + pid 双重校验标 `lost`；确认 pid 已死后只删 meta 所属 id 的锁，任务退回 open |
