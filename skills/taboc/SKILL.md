...
3. **共享额度熔断**：DeepSeek V4 限额即记录全局解除时间并停用整个 OpenCode 池；到期前不试其他免费模型，也不把池故障当任务升级证据。
4. **premium 子 agent 不得强于主 agent**：继承维度写 `inherit-main` 并省略 spawn override；显式值只准严格降档。已知顺序为 `Sol > Luna > Terra`、`Opus > Sonnet > Haiku`，effort 为 `low < medium < high < xhigh < max < ultra`；未知或跨系列继承主模型。免费 OpenCode 不受此上限限制，但禁止因为免费而全开 max。
...
- 侦察证明命中高风险门禁；
- DeepSeek 非额度故障且任务本身另有高风险证据；单纯模型不可用不允许升级。
...
3. 批量启动 OpenCode 前预检；全局 quota state 未到期则写 `[POOL_QUOTA]`、保留队列并报告，禁止调用任何 OpenCode 模型或整批升级：
...
5. 连续启动全部独立 OpenCode 任务；每次检查退出码，首个环境级失败便停止继续启动。只调用 DeepSeek V4；额度错误从结构化事件提取 `retry_after/reset_at` 写入全局状态，缺失时保守冷却 24 小时，绝不回退其他免费模型。launcher 使用一次性 LaunchAgent，任务执行仍并行：
...
6. premium worker 在 spawn 前逐维登记。继承写 `inherit-main`，并省略该维 `model` 或 `reasoning_effort`/`thinking` override；显式值只用于严格降档。主档只接受用户当前回合明示或可信 runtime metadata，禁止猜测、沿用旧登记或手写 assignments 绕过脚本：

   ```bash
   bash <skill-dir>/scripts/register-assignment.sh --repo "<仓库>" \
     --task "<task>" --agent "<agent>" --pool premium \
     --model "<inherit-main|严格更弱模型>" --effort "<inherit-main|严格更低 effort>" \
     [--main-model "<可信主模型>"] [--main-effort "<可信主 effort>"]
   ```
...
用户说“看看任务面板”“任务面板”或“谁在做什么”时，必须运行并原样展示；每项都要显示精确值或可信的 `inherit-main` 继承关系，额度熔断时面板顶部显示解除时间：
...
收尾只核对四件事：OpenCode 可用时是否承接非高风险任务，额度熔断时是否零调用；effort 是否按 `medium/high/max` 分级；premium 的 `inherit-main` 是否省略对应 override、显式值是否严格降档；终态、锁、全量 PASS 和收口白名单是否齐全。
