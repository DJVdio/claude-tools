...
5. 为 OpenCode 写完整 prompt 文件，然后调用：
   ...
   连续启动所有独立任务，每次必须检查 launch 命令退出码；首个环境级失败就停止继续启动。脚本使用 `launchctl bootstrap` 加载 `KeepAlive=false` 的一次性 LaunchAgent，worker 不属于 Codex 命令的进程组，但退出后不会被自动重启。启动时自动登记 `auto-free` 和请求档位，状态文件随后会覆盖为真实模型/档位。
